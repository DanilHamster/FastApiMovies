...
from config import get_jwt_auth_manager, get_settings, BaseAppSettings, get_accounts_email_notificator, get_current_user
...
async def resend_activation_email(
    ...
    activation_link = f"http://127.0.0.1/accounts/activate/?token={new_token.token}&email={user.email}"
...
async def logout_user(
        refresh_token: str = Header(..., description="Refresh token to invalidate"),
        db: AsyncSession = Depends(get_db),
) -> None:
    """
