...
class GenreModel(Base):
    ...
    movies: Mapped[list["MovieModel"]] = relationship(
        "MovieModel", secondary=MoviesGenresModel
    )
...
class StarModel(Base):
    ...
    movies: Mapped[list["MovieModel"]] = relationship(
        "MovieModel", secondary=MovieStarsModel
    )
...
class DirectorModel(Base):
    ...
    name: Mapped[str] = mapped_column(unique=True, nullable=False)
    movies: Mapped[list["MovieModel"]] = relationship(
        "MovieModel", secondary=MovieDirectorsModel
    )
...
class CertificationModel(Base):
    ...
    name: Mapped[str] = mapped_column(unique=True, nullable=False)
    movies: Mapped[list["MovieModel"]] = relationship(
        "MovieModel", back_populates="certification", passive_deletes=True
    )
...
class DislikeModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "movie_id", "user_id", name="unique_movie_user_dislike"
        ),
    )
...
class Comment(Base):
    ...
    replies: Mapped[list["Comment"]] = relationship(
        ...
        primaryjoin=lambda: and_(
            foreign(Comment.target_id) == Comment.id,
            Comment.target_type == literal("movie"),
        ),
    )
...
class MovieModel(Base):
    ...
    comments: Mapped[list["Comment"]] = relationship(
        ...
        lazy="selectin",
        primaryjoin=lambda: and_(
            foreign(Comment.target_id) == MovieModel.id,
            Comment.target_type == literal("movie"),
        ),
    )
    ...
    dislikes = relationship(
        "DislikeModel", back_populates="movie", cascade="all, delete-orphan"
    )
    ...
    certification: Mapped["CertificationModel"] = relationship(
        "CertificationModel", back_populates="movies", passive_deletes=True
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "name", "year", "time", name="unique_movie_name_year_time"
        ),
    )

    def __repr__(self) -> str:
        return (
            f"<Movie(name={self.name!r}, year={self.year}, imdb={self.imdb})>"
        )
